<template>
  <div class="mollie-redirect">
    <div class="payment-status failed" v-if="!paid && !!paymentStatus">
      <div class="paid">
        <p>
          Payment Error <b>{{ paymentStatus.data.status }}</b>
        </p>
        <p>
          tracking code: <span class="code">{{ paymentCode }}</span>
        </p>
      </div>
    </div>
    <div class="pending-payment" v-if="!paid && !goBank">
      <div class="select-payment">
        ...
        <br />
        <div class="al-center">
          <span @click="startPayment()" class="btn reg-btn" :class="[{ disabled: !!!method }]">Volgende stap</span>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    fetchPayment(code) {
      ...
      const options = {
        ...
        headers: {
          ...
          Authorization:
            "Bearer 8Ji0YxXYrynmlIYmIRvqeHTjHMaZstmBgd8eyRLcaxJBHNcUR147Kvs2yO8hkYeVc66nVX1NBhPBSJfZvjwe347bLJWEz1C7dhJ7W87XEghU2cKbJhpoiSmbFP4mPNSO",
        },
        data: form,
      };
    ...
    parseAmount(inpt) {
      let occur = this.countCharsInString(inpt, "."),
        amount = inpt;
      if (occur > 1) {
        let arr = inpt.split(".");
        amount = "";
